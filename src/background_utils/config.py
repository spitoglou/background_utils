...
class Settings(BaseSettings):
    ...
    service_interval_seconds: float = Field(
        default=5.0, ge=0.1, description="Example service tick interval"
    )
    ...
    gmail_password: SecretStr | None = Field(
        default=None, description="Gmail password or app password"
    )
